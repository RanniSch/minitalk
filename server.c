...
/*   Updated: 2022/11/17 15:38:54 by rschlott         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* this function handles the signal SIGINT (control + c), so that if user terminates the program,
it writes a message. */
...
the signals are queued, and they're all delivered. Kill is starting process to use a signal. 
For this specific process (with this process ID) function is called through signal. 
Who I want to send the message to (kill function with the PID and signal) and after what I want to send. */
int main(void)
{
    struct sigaction    sa;
